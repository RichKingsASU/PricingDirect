...
export interface ActualLoadIngestRecord {
  id?: string;
  loadNo?: string;
  containerNo?: string;
  origin: string;
...
export interface ActualsIngestResult {
  updatedExceptions: LaneException[];
  updatedMarkets: MarketSummary[];
...
export interface PlannedAdjustment {
  ...
  status: 'Pending Approval' | 'Active' | 'Scheduled' | 'Rejected';
